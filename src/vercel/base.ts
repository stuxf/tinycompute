...
export async function vercelRequest<T>(
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    ...
    try {
      ...
      console.log(
        `[vercel] ${method} ${path} → ${res.status} (${durationMs}ms)`,
      );
      ...
      if (res.status === 429) {
        ...
        if (attempt < maxRetries) {
          console.log(`[vercel] Rate limited, waiting ${waitMs}ms before retry`);
          await sleep(waitMs);
          continue;
        }
        throw new VercelApiError(429, `Vercel API ${method} ${path}: rate limited`);
      }

      // Retry on 5xx
      if (res.status >= 500) {
        const text = await res.text();
        lastError = new VercelApiError(
          res.status,
          `Vercel API ${method} ${path}: ${res.status} ${text}`,
        );
        if (attempt < maxRetries) {
          const backoff = INITIAL_BACKOFF_MS * Math.pow(2, attempt);
          console.log(
            `[vercel] Server error ${res.status}, retrying in ${backoff}ms (attempt ${attempt + 1}/${maxRetries})`,
          );
          await sleep(backoff);
          continue;
        }
      ...
      if (isAbort || isNetworkError) {
        ...
        console.log(
          `[vercel] ${method} ${path} → ${reason} (${durationMs}ms)`,
        );
        lastError = new VercelApiError(
          isAbort ? 504 : 502,
          `Vercel API ${method} ${path}: ${reason}`,
        );
        if (attempt < maxRetries) {
          const backoff = INITIAL_BACKOFF_MS * Math.pow(2, attempt);
          console.log(
            `[vercel] ${reason}, retrying in ${backoff}ms (attempt ${attempt + 1}/${maxRetries})`,
          );
          await sleep(backoff);
          continue;
        }
      ...
      // Re-throw VercelApiError (already handled above for retryable)
